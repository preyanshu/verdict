...
export default function HistoryPage() {
    ...
    const [userHoldings] = useState<Record<string, { yes: number, no: number }>>({
        'strategy-3': { yes: 250.00, no: 0 }
    });
    ...
    const historicalProposals: MarketStrategy[] = [
        {
            "id": "strategy-3",
            "name": "Digital Gold and Bond Stability",
            "description": "The iShares Bitcoin Trust (IBIT) will exceed 53 AND the iShares 20+ Year Treasury Bond ETF (TLT) will exceed 88.5.",
            "evaluationLogic": "(IBIT > 53 AND TLT > 88.5)",
            "mathematicalLogic": "(asset_IBIT_price > 53 AND asset_TLT_price > 88.5)",
            "usedDataSources": [
                {
                    "id": 12251,
                    "currentValue": 51.875,
            ...
            "resolutionDeadline": 1772202112316,
            "yesToken": {
                "tokenReserve": 2000,
                "volume": 0,
                            ...
                            {historicalProposals.map((proposal) => {
                                ...
                                return (
                                    ...
                                    >
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="flex items-center gap-4">
                                                            <div className="text-[10px] font-mono font-bold text-white/40 uppercase tracking-wider">
                                                                Settled {new Date(proposal.resolutionDeadline).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}
                                                            </div>
                                                            <button
                                                            ...
                                                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
                                                                <div className="p-4 md:p-6 bg-white/[0.02] border border-white/5 rounded-2xl">
                                                                    ...
                                                                    <div className="text-lg md:text-xl font-black text-white tabular-nums">
                                                                        ${(proposal.yesToken.volume + proposal.noToken.volume).toLocaleString()}
                                                                    </div>
                                                                </div>
                                        ...
                                        {(() => {
                                            const holdings = userHoldings[proposal.id];
                                            if (!holdings || (holdings.yes === 0 && holdings.no === 0)) return null;

                                            const isWinner = (holdings.yes > 0 && proposal.winner === 'yes') || (holdings.no > 0 && proposal.winner === 'no');
                                            if (!isWinner) return null;

                                            const amount = holdings.yes > 0 ? holdings.yes : holdings.no;
                                            const type = holdings.yes > 0 ? 'YES' : 'NO';
